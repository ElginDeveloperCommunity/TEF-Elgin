...
type
   ...
   private
      ...
      LAvanco               : integer;               // Avanço de linhas ao finalizar  a impressão
      LModeloImpressoraACBR : TACBrPosPrinterModelo; // Modelo de impressora - Protocolo ESC POS
      LPaginaDeCodigo       : TACBrPosPaginaCodigo;  //  Página de código da impressora
      //------------------------------------------------------------------------
   ...
   public
      ...
      property TpCartaoOperacaoTEF  : TTpCartaoOperacaoTEF read LTpCartaoOperacaoTEF write LTpCartaoOperacaoTEF;  // Tipo de operação do cartão
      ...
      property Avanco               : integer                       read LAvanco               write LAvanco;  // Avanço de linhas ao finalizar  a impressão
      ...
      property PaginaDeCodigo       : TACBrPosPaginaCodigo          read LPaginaDeCodigo       write LPaginaDeCodigo;  //  Página de código da impressora
      //------------------------------------------------------------------------
   end;
  //----------------------------------------------------------------------------
   ...
   begin
      ...
      if ((strtointdef(retorno,0)>=1) and (strtointdef(retorno,0)<=8)) or (retorno='-1') then   // Ocorreu um erro que exige a finalização da operação
         begin
            ...
            //   Mostrar a mensagem de erro na tela
            //---------------------------------------------------------------------
            SA_SalvarLog('FINALIZAR TEF:','Finalizando a transação');   // Salvar LOG
            resultado := UTF8ToString(FinalizarOperacaoTEF(1)); // Finalizando a operação de TEF
