...
//procedure SA_ColetarValor(mensagem,mascara:string;senha : boolean);
...
begin
   if (edtdado.Text<>'') or ((edtdado.Text='') and (AceitaVazio)) then
...
begin
   ...
   if not entrou then
      begin
         ...
         //---------------------------------------------------------------------------
         try
            case TipoTef of   // Selecionando a imagem a ser apresentada durante a transação do TEF
         ...
         case TipoTef of   // Selecionando o título apresentado durante a transação do TEF
           ...
           tpTEFELGIN       : titulo.Caption := 'MKM TEF ELGIN'; // ELGIN
